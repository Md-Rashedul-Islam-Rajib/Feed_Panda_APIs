...
export class CategoriesService {
  ...
  private generateSlug(store_id: string, name: string): string {
  ...
  public async createCategory(createCategoryDto: CreateCategoryDto) {
    ...
    const slug = this.generateSlug(store_id, name);
    ...
    public async updateCategory(id: string, updateCategoryDto: UpdateCategoryDto) {
     // checking if category exists
        ...
        if (updateCategoryDto.name) {
            const category = await this.prisma.category.findUnique({ where: { id } });
            updateCategoryDto['slug'] =this.generateSlug(category!.store_id,updateCategoryDto.name)
        }
